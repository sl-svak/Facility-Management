...
<div class="card" style="border-left: 4px solid var(--primary);">
    <h3 style="margin-top: 0; color: var(--text-main);">Vítejte zpět, <?= htmlspecialchars($_SESSION['first_name'] ?? 'Uživateli') ?>!</h3>
    <p style="margin-bottom: 0; color: var(--text-muted);">Toto je váš řídící panel. Zde vidíte aktuální stav systému a poslední provedené úkony techniků.</p>
</div>

<!-- Tři hlavní statistické karty -->
<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 20px;">
    <div class="card card-primary-top" style="text-align: center; margin-bottom: 0; border-top: 4px solid var(--info);">
        <span class="material-symbols-outlined" style="font-size: 3em; color: var(--info);">precision_manufacturing</span>
        <h2 style="font-size: 2.5em; margin: 10px 0; color: var(--text-main);"><?= $stats['assets_count'] ?></h2>
    ...
    <div class="card" style="border-top: 4px solid var(--success); text-align: center; margin-bottom: 0;">
        ...
        <h2 style="font-size: 2.5em; margin: 10px 0; color: var(--text-main);"><?= $stats['inspections_count'] ?></h2>
    ...
    <div class="card" style="border-top: 4px solid var(--danger); text-align: center; margin-bottom: 0;">
        ...
        <h2 style="font-size: 2.5em; margin: 10px 0; color: var(--text-main);"><?= $stats['open_tickets'] ?></h2>
                        ...
                        <tr>
                            <td style="white-space: nowrap;">
                                ...
                                <?php elseif ($tl['status'] === 'orange'): ?>
                                    <span class="badge" style="background: rgba(243, 156, 18, 0.15); color: var(--warning); display: inline-flex; align-items: center; gap: 5px;">
                            ...
                            <td style="color: var(--text-main);"><?= htmlspecialchars($tl['template_name']) ?></td>
                            <td style="color: var(--text-muted);"><?= $tl['last_inspection'] ? date('d.m.Y', strtotime($tl['last_inspection'])) : '---' ?></td>
                            <td style="font-weight: bold; color: <?= $tl['status'] === 'red' ? 'var(--danger)' : ($tl['status'] === 'orange' ? 'var(--warning)' : 'var(--success)') ?>;">
                                ...
                                <?php if ($tl['status'] !== 'green' && $tl['last_inspection']): ?>
                                    <br><span style="font-size: 0.8em; font-weight: normal; color: var(--text-muted);">(Zbývá <?= $tl['days_remaining'] ?> dní)</span>
                                <?php endif; ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <table class="table">
                ...
                <tbody>
                    <?php foreach ($recent_inspections as $insp): ?>
                        <tr>
                            <td style="color: var(--text-muted);"><?= date('d.m.Y H:i', strtotime($insp['created_at'])) ?></td>
                            <td style="font-weight: bold; color: var(--primary);"><?= htmlspecialchars($insp['asset_name']) ?></td>
                            <td style="color: var(--text-main);"><?= htmlspecialchars($insp['template_name']) ?></td>
                            <td style="color: var(--text-muted);"><?= htmlspecialchars($insp['first_name'] . ' ' . $insp['last_name']) ?></td>
                            <td>
                                <?php if ($insp['status'] === 'OK'): ?>
                                    <span class="badge badge-closed">V POŘÁDKU</span>
                                <?php elseif ($insp['status'] === 'Odstaveno'): ?>
                                    <span class="badge" style="background: rgba(149, 165, 166, 0.2); color: var(--text-muted);">ODSTÁVKA</span>
                                <?php else: ?>
